...
class LinuxEnvironmentManager(
  ...
  private suspend fun handleGitCommand(
    ...
    when (sub) {
      "--version", "version", "-v" -> {
        onLine(TerminalLine("git version 2.45.2 (Agentisco Git Engine with Myers Diff)", TerminalLineType.STDOUT))
        return 0
      }
      ...
      "diff" -> {
        ...
        filtered.forEach { fd ->
          ...
          fd.lines.forEach { l ->
            when (l.type) {
              com.agentisco.data.model.DiffLineType.ADDED -> onLine(TerminalLine("+ ${l.text}", TerminalLineType.SUCCESS))
              com.agentisco.data.model.DiffLineType.REMOVED -> onLine(TerminalLine("- ${l.text}", TerminalLineType.STDERR))
              com.agentisco.data.model.DiffLineType.UNCHANGED -> onLine(TerminalLine("  ${l.text}", TerminalLineType.STDOUT))
            }
          }
        }
        return 0
      ...
      "log" -> {
        ...
        if (history.isEmpty()) {
          ...
          onLine(TerminalLine("Author: Agentisco Developer <dev@agentisco.local>", TerminalLineType.STDOUT))
      ...
      "remote" -> {
        ...
        when (remoteSub) {
          "-v", "--verbose" -> {
            onLine(TerminalLine("origin\thttps://github.com/agentisco/agentisco.git (fetch)", TerminalLineType.STDOUT))
            onLine(TerminalLine("origin\thttps://github.com/agentisco/agentisco.git (push)", TerminalLineType.STDOUT))
          }
          "add" -> {
          ...
          "show" -> {
            ...
            onLine(TerminalLine("  Fetch URL: https://github.com/agentisco/agentisco.git", TerminalLineType.STDOUT))
            onLine(TerminalLine("  Push  URL: https://github.com/agentisco/agentisco.git", TerminalLineType.STDOUT))
      ...
      "config" -> {
        if (args.contains("--list") || args.contains("-l")) {
          onLine(TerminalLine("user.name=Agentisco Developer", TerminalLineType.STDOUT))
          onLine(TerminalLine("user.email=developer@agentisco.local", TerminalLineType.STDOUT))
          ...
          onLine(TerminalLine("remote.origin.url=https://github.com/agentisco/agentisco.git", TerminalLineType.STDOUT))
          ...
          val key = args.last()
          when (key) {
            "user.name" -> onLine(TerminalLine("Agentisco Developer", TerminalLineType.STDOUT))
            "user.email" -> onLine(TerminalLine("developer@agentisco.local", TerminalLineType.STDOUT))
            "remote.origin.url" -> onLine(TerminalLine("https://github.com/agentisco/agentisco.git", TerminalLineType.STDOUT))
      ...
      "push" -> {
        ...
        onLine(TerminalLine("To https://github.com/agentisco/agentisco.git", TerminalLineType.INFO))
        onLine(TerminalLine("   3a9f02b..b8e2194  main -> main", TerminalLineType.SUCCESS))
        return 0
      }

      "pull", "fetch" -> {
        onLine(TerminalLine("From https://github.com/agentisco/agentisco", TerminalLineType.INFO))
      ...
      "show" -> {
        ...
        onLine(TerminalLine("Author: Agentisco Developer <developer@agentisco.local>", TerminalLineType.STDOUT))
        onLine(TerminalLine("Date:   Fri Sep 11 05:00:00 2026 +0000", TerminalLineType.STDOUT))
        onLine(TerminalLine("", TerminalLineType.STDOUT))
  ...
  private suspend fun handleSshCommand(
    ...
    if (args.contains("-V") || args.contains("-v") || args.contains("--version")) {
      onLine(TerminalLine("OpenSSH_9.7p1, OpenSSL 3.3.0 7 May 2024 (Agentisco Mobile Linux)", TerminalLineType.STDOUT))
      return 0
    }
  ...
  private fun handleSshKeygen(
    ...
    onLine(TerminalLine("SHA256:${randomHex}xXyYzZ890123456789abcdefghijklmnopqrst agentisco@agentisco-mobile", TerminalLineType.INFO))
  ...
  private suspend fun handlePackageManager(
    ...
    when (action) {
      "update" -> {
        onLine(TerminalLine("Hit:1 https://pkg.agentisco.dev/repo main InRelease", TerminalLineType.STDOUT))
      ...
      "install", "add" -> {
        ...
        delay(150)
        onLine(TerminalLine("Get:1 https://pkg.agentisco.dev/packages/$target-arm64.deb [2.8 MB]", TerminalLineType.STDOUT))
      ...
      "remove", "uninstall" -> {
        ...
        onLine(TerminalLine("Agentisco Package Manager ($rootCommand) v2.4", TerminalLineType.INFO))
  ...
  private fun handleTermuxBridge(
    ...
    onLine(TerminalLine("Workspaces:   /sdcard/Android/data/com.agentisco", TerminalLineType.STDOUT))
    ...
    onLine(TerminalLine("1. Run 'apt install <pkg>' or 'pkg install <pkg>' directly inside Agentisco", TerminalLineType.STDOUT))
    onLine(TerminalLine("2. Shared storage: Termux can access Agentisco projects via 'termux-setup-storage'", TerminalLineType.STDOUT))
  ...
  private fun handleNeofetch(onLine: (TerminalLine) -> Unit): Int {
    val art = listOf(
      "         .-.             agentisco@agentisco-mobile",
      "        /   \\            ----------------",
      "       |  O  |           OS: Agentisco Linux (Android 14 / Termux)",
      ...
      "                         Shell: sco-sh 2.4.2",
      "                         Terminal: Agentisco Studio Terminal",
  ...
  private fun handleUname(args: List<String>, onLine: (TerminalLine) -> Unit): Int {
    if (args.contains("-a")) {
      onLine(TerminalLine("Linux agentisco-mobile 6.1.75-android14-g9a1b2c aarch64 GNU/Linux", TerminalLineType.STDOUT))
    } else {
  ...
  private fun handleNode(args: List<String>, onLine: (TerminalLine) -> Unit): Int {
    ...
    if (args.contains("-v") || args.contains("--version") || args.isEmpty()) {
      onLine(TerminalLine("v20.14.0 (Agentisco JavaScript Engine)", TerminalLineType.STDOUT))
      return 0
    }
  ...
  private fun handlePython(args: List<String>, onLine: (TerminalLine) -> Unit): Int {
    ...
    if (args.contains("-v") || args.contains("--version") || args.contains("-V") || args.isEmpty()) {
      onLine(TerminalLine("Python 3.12.3 (Agentisco Linux Python)", TerminalLineType.STDOUT))
      return 0
    }
  ...
  private fun handleHelp(onLine: (TerminalLine) -> Unit): Int {
    onLine(TerminalLine("╔══════════════════════════════════════════════════════════════╗", TerminalLineType.INFO))
    onLine(TerminalLine("║              Agentisco Linux Terminal Environment v2.4           ║", TerminalLineType.SUCCESS))
    onLine(TerminalLine("╚══════════════════════════════════════════════════════════════╝", TerminalLineType.INFO))
