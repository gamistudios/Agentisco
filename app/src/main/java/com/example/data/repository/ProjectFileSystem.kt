...
class ProjectFileSystem(private val baseDir: File) {
  ...
  fun createProject(name: String, description: String): Project {
    ...
    File(projDir, ".sco_meta").writeText(description.ifBlank { "Created in ScoOS" })
  ...
  private fun initializeDefaultProjectsIfEmpty() {
    val scoDir = File(baseDir, "scospace")
    if (!scoDir.exists() || (scoDir.list()?.isEmpty() == true)) {
      scoDir.mkdirs()
      File(scoDir, ".sco_name").writeText("ScoSpace")
